...
public class ClientManageAction extends BaseAction
{
    ...
    throws Exception
  {
    ...
    logger.info(userinfo.getUiUsername() + " 在" + new Date() + "----执行查询客户功能.");
    return "success";
  }
    ...
    throws Exception
  {
    ...
    HttpSession session = ServletActionContext.getRequest().getSession();
    Userinfo userinfo = (Userinfo)session.getAttribute("userinfo");
    logger.info(userinfo.getUiUsername() + " 在" + new Date() + "----执行删除客户功能.删除客户的Id为：" + this.clientId + "。");
    addSuccessInfo("删除编号为 ：" + this.clientId + "的客户成功。");
    return "success";
  }
    ...
    throws Exception
  {
    ...
    if (this.changeString == null) {
      logger.info(userinfo.getUiUsername() + " 在" + new Date() + "----执行更新客户功能.更新客户失败。");
      addActionError("更新内容为空！");
      return "input";
    }
    IClientBO clientBo = (IClientBO)SpringBeanFactory.getBean("clientBo");
    ...
    if (client == null) {
      logger.info(userinfo.getUiUsername() + " 在" + new Date() + "----执行更新客户功能.更新客户失败。");
      addActionError("没有发现编号为 ：" + this.clientUpdateId + "的客户！");
      return "input";
    }

    String decrypt = MyTools.vigenere(this.changeString, MyTools.getUpKey(this.clientUpdateId, MyTools.STRING), MyTools.ENCRYPT_MODE);
    client.setClientLimit(decrypt);
    clientBo.updateClient(client);
    logger.info(userinfo.getUiUsername() + " 在" + new Date() + "----执行更新客户功能.更新客户的Id为：" + this.clientUpdateId + "，更新客户成功。");
    addSuccessInfo("修改编号为 ：" + this.clientUpdateId + "的客户成功。");
    return "success";
  }
