...
<head>
	...
	<script type="text/javascript" src="../dwr/interface/UpdateEmpsalaryacct.js" ></script>
...
<body onload="initDisabledField()">
	...
	<s:form id="updateesa" name="updateesa" action="updateesa" namespace="/compensation" method="POST">
		...
		<table width="100%" cellpadding="0" border="0">
			<tr><td>
				...
				<s:if test="viewType==0">
				    &nbsp;<input type="button" class='button' value=" 更新 " onclick="updateSalaryConf();" />			
					&nbsp;<input type="button" class='button' value=" 另存 " onclick="return saveto();" />
				</s:if>
				&nbsp;<input type="button" class='button' value=" 取消 " onclick="if(confirm('您确实要取消本次操作吗？'))window.location='searchesa.action'" />
				&nbsp;<input type="button" class='button' value="导出帐套" id="btnOutput" onclick="export_check('export');" />
			 </td></tr>
		</table>
		...
		<input type="hidden" id="empsalaryacct.id" name="empsalaryacct.id" value="<s:property value='empsalaryacct.id'/>"/>
		<input type="hidden" id="id" name="id" value="<s:property value='empsalaryacctversion.id'/>"/>
		 <input type="hidden" id="empsalaryacctversion.id" name="empsalaryacctversion.id" value="<s:property value='empsalaryacctversion.id'/>"/>
		 <input type="hidden" id="empsalaryacctversion.esavCreateTime" name="empsalaryacctversion.esavCreateTime" value="<s:property value='empsalaryacctversion.esavCreateTime'/>"/>
		 <input type="hidden" id="empsalaryacctversion.esavCreateBy" name="empsalaryacctversion.esavCreateBy" value="<s:property value='empsalaryacctversion.esavCreateBy'/>"/>
		 <input type="hidden" id="empsalaryacctversion.esavValidFrom" name="empsalaryacctversion.esavValidFrom" value="<s:property value='empsalaryacctversion.esavValidFrom'/>"/>
		<input type="hidden" id="EMPSALARY_ACCT_LIMIT" value="<s:property value="@com.hr.base.Constants@EMPSALARY_ACCT_LIMIT"/>"/>
		<input type="hidden" id="viewType" name="viewType" value="<s:property value='viewType' />"/>
		<table id="esaContentTable" cellpadding="0" cellspacing="0" width="100%" border="0" class="gridtable2">	
			...
			<s:bean name="org.apache.struts2.util.Counter" id="rowcounter">
  			<s:param name="first" value="1"/><s:param name="last" value="dataDefIds.length>=10?dataDefIds.length+1:10"/>
			</s:bean>
			...
			<td  align="center">
				...
				<select id="dataTypes" name="dataTypes" onchange="editTr=this.parentNode.parentNode;changeItems(this.value)" value="<s:property value='dataTypes[#rowstatus.count-1]' />"> 
			...
			<td  align="center">
				<s:if test="viewType==1">
					<input type="text" id="dataCalcs" readOnly value="<s:property value="dataCalcs[#rowstatus.count-1]"/>" name="dataCalcs"  size="40"/>
				</s:if>
				<s:else>
			 	<input type="text" id="dataCalcs" onfocus="changetono();" onblur="formulaVali(this, this.value)" onclick="currentTr=this.parentNode.parentNode;editTr=currentTr;" value="<s:property value="dataCalcs[#rowstatus.count-1]"/>" onclick="currentTr=this;editTr=currentTr;"  name="dataCalcs"  size="40"/>
				</s:else>
			</td>			
			<td  align="center">
			...
			<td align="center">
				<s:if test="viewType==0">
						<img src="../resource/images/delete_inline.gif" onMouseOver="cursor:'hand';" 
...
//更新帐套信息前的检查；jet miao, 2009-04-17
  function updateSalaryConf(){
    //项目填写合格性检查；
    if(!valadi()){
        return;
    }
    var acctversionId = document.getElementById("empsalaryacctversion.id").value;
    var dataDefIds = document.getElementsByName("dataDefIds");
    var defIdsArray = new Array();
    for(var i=0; dataDefIds!=null && i < dataDefIds.length; i++){
        defIdsArray[i] = dataDefIds[i].value;
    }
    UpdateEmpsalaryacct.pageGetDiff(acctversionId, defIdsArray, checkCallBack);
}
//回调函数；
function checkCallBack(msg){
	...
	try{
		...
		if(retcode == 'WARNING') {
            document.getElementById('updateesa').submit();
		}
	}catch(e){alert(e);}
}
...
function changeItems(type) {
		...
		if(type == '0') {
			for(var i=0;i<editSelect.options.length;i++) {
				editSelect.options[i] = new Option('','',false,false);
			}
			return;
		}
...
function getDefault(esddId) {
		...
		function changItemCallBack(msg) {
			...
			var dataName = editTr.getElementsByTagName("input")[1];
			var dataDefId = editTr.getElementsByTagName("select")[1];
			dataName.value = dataDefId.options[dataDefId.selectedIndex].text;
...
function valadi() {
		...
		for(var i=0;i<itemIds.length;i++) {
			...
			for(var j=0;j<itemIds.length;j++) {
				//alert(itemIds[i].value+"   "+itemIds[j].value);
...
//如果项目种类为不启用，则disable本行，否则disabled为false 
function initDisabledField() {
		...
		for(var i=0;i<items.length;i++) {
			...
			if(document.getElementById('viewType').value=='1') {
...
function saveto(){
    if(!confirm("您确定要另存帐套为新版本吗？")){
        return false;
    }
   	if(!valadi()) {
   		return false;
   	}
    document.getElementById('updateesa').action='addAcctVersion.action';
    document.getElementById('searchOrExport').value = "";
    document.getElementById('updateesa').submit();
}
function export_check(){
    document.getElementById('searchOrExport').value = 'export';
    document.forms[0].action = 'updateesainit.action';
